...
function formatDateToVarchar(date) {
...
app.post("/api/formaciones", async (req, res) => {
  let fecha_inicio = formatDateToVarchar(req.body?.fecha_inicio || null);
  let fecha_final = formatDateToVarchar(req.body?.fecha_final || null);
  ...
  try {
    ...
    if (fecha_inicio && fecha_final) {
      query += ` WHERE STR_TO_DATE(FECHA, '%d/%m/%Y') BETWEEN STR_TO_DATE('${fecha_inicio}', '%d/%m/%Y') AND STR_TO_DATE('${fecha_final}', '%d/%m/%Y')`;
    } else if (fecha_inicio) {
      query += ` WHERE STR_TO_DATE(FECHA, '%d/%m/%Y') >= STR_TO_DATE('${fecha_inicio}', '%d/%m/%Y')`;
    } else if (fecha_final) {
      query += ` WHERE STR_TO_DATE(FECHA, '%d/%m/%Y') <= STR_TO_DATE('${fecha_final}', '%d/%m/%Y')`;
    }
...
app.post("/api/consentimientos", async (req, res) => {
  let fecha_inicio = req.body?.fecha_inicio || null;
  let fecha_final = req.body?.fecha_final || null;
